...
def index():
    ...
    files.sort(key=lambda fl: os.path.getmtime(os.path.join(app.config['UPLOAD_DIRECTORY'], fl)))
    images = []
...
def upload():
    ...
    try:
        if file:
            ...
            yolo_command = 'python3 yolov7/detect.py --weights yolov7/runs/train/yolov733/weights/best.pt \
                --conf 0.5 --no-trace --save-txt --img-size 640 --source ' + upload_file_name
            os.system(yolo_command)
            move_command = 'mv runs/detect/exp/{0} {1}'.format(file_name,
                                                               os.path.join(app.config['PREDICTION_DIR_YOLO'],
                                                                            file_name))
